 ...
 * - Memory: delegates durable state changes to ChunksService.
...
export class ChunksProcessor extends WorkerHost {
   ...
   * - Memory: the chunks service reads and writes durable chunk state.
   */
  async process(job: BullJob<AnalyzeChunkJobData>) {
    if (job.name !== CHUNK_QUEUE_JOB_NAME) return;

    await this.chunksService.processQueuedChunk(job.data.chunkId);
  }
  ...
  async onCompleted(job: BullJob<AnalyzeChunkJobData>) {
    ...
    await this.chunksService.finalizeJobForChunk(job.data.chunkId);
  }
   ...
   * - Memory: failed chunk state has already been persisted by the chunks service.
